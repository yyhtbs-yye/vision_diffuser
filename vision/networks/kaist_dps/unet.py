...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []
        if timesteps.ndim == 0:
            timesteps = timesteps[None].repeat(x.shape[0])


        # print(next(self.time_embed.parameters()).device)
        # print(timestep_embedding(timesteps, self.model_channels).device)
    ...
    def from_config(cls, config):
        """
        Create a UNetModel from a configuration dictionary.
        
        Args:
            config: A dictionary containing the model configuration.
                   Must include 'image_size', 'num_channels', 'num_res_blocks'.
                   May include 'pretrain' for checkpoint path.
                   
        Returns:
            A UNetModel instance.
        """
        ...
        channel_mult = config.get('channel_mult', "")
        if channel_mult == "":
            if image_size == 512:
        ...
        model = cls(
            ...
            out_channels=(3 if not config.get('learn_sigma', False) else 6),
            ...
            num_classes=(NUM_CLASSES if config.get('class_cond', False) else None),
