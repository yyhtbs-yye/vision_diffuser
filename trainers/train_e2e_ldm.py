...
from vision.models.generation.e2e_latent_diffusing import End2endTrainableLatentDiffusion
...
config_path = "configs/e2e_ldm_train_64.yaml"
...
checkpoint_path = "work_dirs/e2e_ldm/ldm_64/checkpoints/epoch=10-val/noise_mse=0.0233.ckpt"
if checkpoint_path and os.path.exists(checkpoint_path):
    print(f"Loading model from checkpoint: {checkpoint_path}")
    model = End2endTrainableLatentDiffusion.load_from_checkpoint(
        checkpoint_path, 
    ...
    model = End2endTrainableLatentDiffusion(
        model_config=model_config,
        train_config=train_config,
        validation_config=validation_config
    )
...
checkpoint_callback = ModelCheckpoint(
    dirpath=os.path.join(logging_config['log_dir'], 
                         logging_config['experiment_name'], 'checkpoints'),
